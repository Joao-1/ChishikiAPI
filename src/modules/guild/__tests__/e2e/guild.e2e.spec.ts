...
describe("Guild", () => {
	...
	describe("Guild Register", () => {
		it("should return a new guild.", async () => {
			const response = await req.post("/guilds").send({ id: "751646711251730452" });
		...
		it("should return an error when trying to register a guild that is already registered.", async () => {
			...
			const response = await req.post("/guilds").send({ id: "1234" });
	...
	describe("Get Guild", () => {
		it("should return one guild", async () => {
			...
			const getOneGuildResponse = await req.get("/guilds").send();
		...
		it("should return two guilds of five", async () => {
			...
			const getTwoOfFiveGuildResponse = await req.get("/guilds").query({ offset: 3, limit: 2 });
